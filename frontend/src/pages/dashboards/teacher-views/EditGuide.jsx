...
const EditGuide = () => {
  ...
  useEffect(() => {
    const fetchGuide = async () => {
      try {
        ...
        setSections(data.sections && data.sections.length > 0 ? data.sections : [{ heading: "", body: "" }]);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.error || error.message || "Failed to update guide");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Edit Guide</h1>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end pt-4 pb-8">
            <button
              type="submit"
              disabled={loading}
              className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-blue-200 transition transform active:scale-95 disabled:opacity-50 flex items-center gap-2"
            >
              {loading ? (
                <>
